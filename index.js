...
function addGamesToPage(games) {
    ...
    for(let i = 0; i < games.length; i++){
        ...
        newGameCard.innerHTML = `<div class="game-card"> 
...
const displayedString = `A total of $${totalMoneyRaised.toLocaleString()} has been raised for ${allGames} ${allGames === 1 ? "game." : "games."}` +
  ` Currently, ${totalUnfundedGames} ${totalUnfundedGames === 1 ? "game remains" : "games remain"} unfunded. We need your help to fund these amazing games!`;
displayStr.textContent = displayedString;
...
displayFirstGame.innerHTML = `${firstGame.name}`; 
displaySecondGame.innerHTML = `${secondGame.name}`;
firstGameContainer.appendChild(displayFirstGame);
secondGameContainer.appendChild(displaySecondGame);
